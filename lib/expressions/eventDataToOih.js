...
module.exports.getExpression = function(msg)
{
  const jsonataExpression =
  {
    collaborationElement: {
      header: {
        from: "",
        to: jsonata(`$map(msg.body.attendees, function($v) {
      ...
      date: {
        ...
        day: jsonataMoment('$moment(msg.body.createdDateTime).format("dddd")').evaluate(),
        time: jsonataMoment('$moment(msg.body.createdDateTime).format("HH:mm")').evaluate()
      },
      properties: {
      ...
      attachments: jsonata(`$map(msg.body.attachments, function($v) {
        {
          type: $v.contentType,
          size: $string($v.size)
        }
    ...
    contacts: jsonata(`$map(msg.body.attendees, function($v) {
      {
        name: "",
    ...
    location: msg.body.location.displayName,
